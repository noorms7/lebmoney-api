import { PrismaClient, Prisma, CurrencyAccount } from "@prisma/client";

const prisma = new PrismaClient();

async function main() {
  // Platform fee-revenue account — every Send Money fee is routed here so
  // the ledger stays balanced (see TransactionsService).
  ...
  // Demo-only bootstrap: credits Rami with a starting balance so the Send
  // Money flow is actually testable without a real funding rail wired up.
  // A real top-up settles against an external bank/card rail account, not
  // the fee-revenue account — this shortcut is for local demo purposes only.
  ...
  const ramiUsd = ramiWallet.currencyAccounts.find((a: CurrencyAccount) => a.currency === "USD")!;
  const systemUsd = systemWallet.currencyAccounts.find((a: CurrencyAccount) => a.currency === "USD")!;
  ...
  // Demo-only: a flagged transaction so Transaction Monitoring isn't empty
  // on a fresh seed. A real flag would be raised by a fraud-rules engine,
  // not manually inserted like this.
